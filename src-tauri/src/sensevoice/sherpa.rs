...
mod imp {
    use crate::sensevoice::SenseVoiceError;
    ...
    pub fn get_models_root() -> Option<PathBuf> {
        models_root_slot().lock().ok().and_then(|slot| slot.clone())
    }
        ...
        F: FnMut(&str, Option<u8>, Option<u64>, Option<u64>),
    {
        ...
        on_progress(
            &format!("Downloading {MODEL_ARCHIVE_NAME}"),
            Some(0),
            Some(0),
            None,
        );
        ...
        on_progress(
            "Extracting Sherpa-ONNX SenseVoice model",
            Some(95),
            None,
            None,
        );
    ...
    pub fn load_runtime(models_root: &Path, language: &str) -> Result<(), SenseVoiceError> {
        ...
        let should_reload = runtime.as_ref().is_none_or(|current| {
            current.model_dir != model_dir || current.language != normalized_language
        });
        if !should_reload {
            return Ok(());
        }
    ...
    pub fn transcribe_wav(
        ...
        language: &str,
        audio_path: &Path,
    ) -> Result<SherpaResult, SenseVoiceError> {
        ...
        let result = stream.get_result().ok_or_else(|| {
            SenseVoiceError::Parse("Sherpa recognition result is empty".to_string())
        })?;
        let alignment = result
            .timestamps
            .as_ref()
            .map(|timestamps| TranscriptionAlignment {
    ...
    fn create_recognizer(
        model_dir: &Path,
        language: &str,
    ) -> Result<OfflineRecognizer, SenseVoiceError> {
        ...
        OfflineRecognizer::create(&config).ok_or_else(|| {
            SenseVoiceError::Process(
                "failed to create Sherpa-ONNX SenseVoice recognizer".to_string(),
            )
        })
    }
        ...
        F: FnMut(&str, Option<u8>, Option<u64>, Option<u64>),
    {
        ...
        let file =
            File::create(archive_path).map_err(|err| SenseVoiceError::Io(err.to_string()))?;
        ...
        loop {
            ...
            let percent =
                total.map(|value| ((downloaded as f64 / value as f64) * 90.0).round() as u8);
    ...
    fn extract_archive(archive_path: &Path, models_root: &Path) -> Result<(), SenseVoiceError> {
        let archive_file =
            File::open(archive_path).map_err(|err| SenseVoiceError::Io(err.to_string()))?;
...
mod imp {
    ...
    fn unsupported_target_error() -> SenseVoiceError {
        SenseVoiceError::Config(
            "Sherpa-ONNX SenseVoice is not supported on Windows ARM64 builds".to_string(),
        )
    }
    ...
    pub fn prepare_model<F>(_models_root: &Path, _on_progress: F) -> Result<(), SenseVoiceError>
    where
        F: FnMut(&str, Option<u8>, Option<u64>, Option<u64>),
    {
        Err(unsupported_target_error())
    }

    pub fn load_runtime(_models_root: &Path, _language: &str) -> Result<(), SenseVoiceError> {
        Err(unsupported_target_error())
    }
